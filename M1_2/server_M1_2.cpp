 ...
 * Single Process , Multiple Thread , Solution with Semaphore
 * 
 * 
 * 
 * 
...
void *serveRequest (void *sock)	 {		
   ...
   while(1)
   {
		...
		{	//logic to search in file and send that to client			
			...
			//not require to wait on resource as only reading , if we do it will make resource 0 and next reader will block
...
int main(int argc , char *argv[]  )
{
	...
	if( bind(sockfd , (struct sockaddr *) &serv_addr, sizeof(serv_addr)) < 0 ) {
		close(sockfd);
		cout<<"ERROR on binding"<<endl;	    	exit(1);
	}
	
	//start listening for the clients
	listen(sockfd , 5);
	clilen = sizeof(cli_addr) ;
	cout<<"listening for clients request on port "<< portno<<endl;			
	pthread_t threads[MAX_THREAD];
	sem_init(&mutex, 0, 1);	//initializing unnamed semaphore
	...
	while(1){
		...
		if (newsockfd < 0) {
			cout<<"ERROR on accept"<<endl;	        exit(1);
		}
